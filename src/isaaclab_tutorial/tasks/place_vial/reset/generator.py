...
class _Generator:
    ...
    def _tabletop_pose(self, phase: int) -> torch.Tensor:
        ...
        pose[:, :2] += (
            torch.empty((self.num_envs, 2), device=self.device).uniform_(-1.0, 1.0, generator=self.random) * half_range
        )
    ...
    def generate(self) -> tuple[dict[str, torch.Tensor], dict[str, int]]:
        ...
        for phase, phase_name in enumerate(PHASE_NAMES):
            ...
            while (
                ...
                if attempts % (4 * self.num_envs) == 0 and (accepted < self.cfg.poses_per_phase or not branch_ready()):
                    summary = {
