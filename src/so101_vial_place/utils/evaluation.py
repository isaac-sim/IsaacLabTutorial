    ...
    def recorded_step(self, actions):
        ...
        if step_count in capture_steps:
            ...
            if ids.numel():
                ...
                vial = env.scene["vial"]
                captured["joint_position"].append(_tensor(robot.data.joint_pos)[ids].detach().cpu())
                captured["joint_target"].append(_tensor(robot.data.joint_pos_target)[ids].detach().cpu())
                ...
                captured_difficulty.append(torch.full((ids.numel(),), step_count / 900.0, dtype=torch.float32))
        if step_count == max(capture_steps):
            ...
            bridge = {name: torch.cat(batches) for name, batches in captured.items()}
...
def install_canonical_policy_dataset_recorder() -> list[str]:
    ...
    return _install_policy_dataset_recorder(Path("checkpoints/datasets/vision_specialists/canonical.pt"), phase=0)
...
def install_transport_policy_dataset_recorder() -> list[str]:
    ...
    return _install_policy_dataset_recorder(Path("checkpoints/datasets/vision_specialists/transport.pt"), phase=5)


def install_phase1_policy_dataset_recorder() -> list[str]:
    return _install_policy_dataset_recorder(Path("checkpoints/datasets/vision_specialists/phase1.pt"), phase=1)


def install_phase2_policy_dataset_recorder() -> list[str]:
    return _install_policy_dataset_recorder(Path("checkpoints/datasets/vision_specialists/phase2.pt"), phase=2)


def install_phase3_policy_dataset_recorder() -> list[str]:
    return _install_policy_dataset_recorder(Path("checkpoints/datasets/vision_specialists/phase3.pt"), phase=3)


def install_phase4_policy_dataset_recorder() -> list[str]:
    return _install_policy_dataset_recorder(Path("checkpoints/datasets/vision_specialists/phase4.pt"), phase=4)


def _install_policy_dataset_recorder(output: Path, *, phase: int, target_steps: int | None = None) -> list[str]:
    ...
    *,
    ...
    def counted_step(self, actions):
        ...
        if once_per_env and self.num_envs != target:
            raise RuntimeError(
                f"Exact once-per-environment audit requires --num_envs {target}; received {self.num_envs}."
            )
            ...
            if completed >= target:
                ...
                if insertion_states:
                    ...
                    insertion_summary = {
                        ...
                        "transport_clearance_rate": float((state[:, 5] >= RACK_CLEARANCE_HEIGHT).float().mean()),
